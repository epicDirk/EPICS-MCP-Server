...
async def test_validate_pvs_all_connected():
    with patch(
...
async def test_validate_pvs_mixed():
    async def _mock_pv_get(name, timeout):
        if name == "PV:1":
...
async def test_validate_pvs_no_input():
...
async def test_validate_pvs_file_path_no_core():
    """file_path mode without phoebus_mcp_core raises MISSING_DEPENDENCY."""
    with patch.dict("sys.modules", {"phoebus_mcp_core": None, "phoebus_mcp_core.bob_parser": None}):
        with pytest.raises(EpicsError, match="phoebus-mcp-core") as exc_info:
