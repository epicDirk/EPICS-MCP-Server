...
async def test_discover_wildcard():
    result = await _discover_pvs("TEST:*")

    assert result["total"] == 0
    assert "ChannelFinder" in result["note"]


async def test_discover_concrete_found():
    with patch(
    ...
    assert result["pvs"][0]["status"] == "found"
    assert result["pvs"][0]["value"] == 42.0


async def test_discover_concrete_not_found():
    with patch(
        "epics_pv_mcp.tools.discover.pv_get",
        new_callable=AsyncMock,
    ...
    assert result["total"] == 0
    assert result["pvs"][0]["status"] == "not_found"
